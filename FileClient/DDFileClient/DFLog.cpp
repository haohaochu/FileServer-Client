...
{
	UINT index = 0;
	CDDFileClientDlg* parant = (CDDFileClientDlg*)this->GetParentsHandle();
...
/*public */void CDFLogMaster::DFLogSend(CString& msg, SEVERITY severity)
{
	...
	if (severity >= DF_SEVERITY_WARNING)
	{
		LogSend(msg.GetBuffer(0), severity);
		msg.ReleaseBuffer();
	}
